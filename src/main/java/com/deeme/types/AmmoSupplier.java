...
public class AmmoSupplier implements PrioritizedSupplier<SelectableItem> {
    ...
    List<String> damageOrder = Arrays.asList(Laser.RCB_140.getId(), Laser.PIB_100.getId(), Laser.MCB_50.getId(),
            Laser.MCB_25.getId(),
            Laser.LCB_10.getId());
    ...
    public SelectableItem get() {
        try {
            return items.getItems(ItemCategory.LASERS).stream()
                    .filter(item -> item.isUsable() && item.isAvailable() && item.getQuantity() > 20)
